...
@section('content')
    ...
    <div class="col-12">
        <div class="bg-light rounded h-100 p-4">
            <h6 class="mb-4">{{ trans('global.edit') }} {{ trans('cruds.comment.title_singular') }}</h6>
            <form action="{{ route("admin.comments.update", [$comment->id]) }}" method="POST"
                ...
                <div class="form-group {{ $errors->has('ticket_id') ? 'has-error' : '' }}">
                    ...
                    <select name="ticket_id" id="ticket" class="form-control select2">
                        @foreach($tickets as $id => $ticket)
                            <option
                                value="{{ $id }}" {{ (isset($comment) && $comment->ticket ? $comment->ticket->id : old('ticket_id')) == $id ? 'selected' : '' }}>{{ $ticket }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('author_name') ? 'has-error' : '' }}">
                    <label for="author_name">{{ trans('cruds.comment.fields.author_name') }} <strong class="text-danger">*</strong></label>
                ...
                <div class="form-group {{ $errors->has('author_email') ? 'has-error' : '' }}">
                    <label for="author_email">{{ trans('cruds.comment.fields.author_email') }} <strong class="text-danger">*</strong></label>
                ...
                <div class="form-group {{ $errors->has('user_id') ? 'has-error' : '' }}">
                    ...
                    <select name="user_id" id="user" class="form-control select2">
                        @foreach($users as $id => $user)
                            <option
                                value="{{ $id }}" {{ (isset($comment) && $comment->user ? $comment->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('comment_text') ? 'has-error' : '' }}">
                    <label for="comment_text">{{ trans('cruds.comment.fields.comment_text') }} <strong class="text-danger">*</strong></label>
