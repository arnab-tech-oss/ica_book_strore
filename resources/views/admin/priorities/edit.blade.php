...
@section('content')
    ...
    <div class="col-12">
        <div class="bg-light rounded h-100 p-4">
            <h6 class="mb-4">{{ trans('global.edit') }} {{ trans('cruds.priority.title_singular') }}</h6>
            <form action="{{ route("admin.priorities.update", [$priority->id]) }}" method="POST"
                  enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                    <label for="name">{{ trans('cruds.priority.fields.name') }} <strong class="text-danger">*</strong></label>
                ...
                <div class="form-group {{ $errors->has('color') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="color" name="color" class="form-control colorpicker"
                           value="{{ old('color', isset($priority) ? $priority->color : '') }}">
                    @if($errors->has('color'))
                        <em class="invalid-feedback">
                            {{ $errors->first('color') }}
                        </em>
