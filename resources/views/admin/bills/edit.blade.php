...
@section('content')
    ...
    <div class="col-12">
        <div class="bg-light rounded h-100 p-4">
            <h6 class="mb-4">{{ trans('global.edit') }} {{ trans('cruds.bills.title_singular') }}</h6>
            <div class="alert alert-danger errorMessage_div" style="display: none;">
                <span class="errorMessage"></span>
            </div>
            <form action="{{ route("admin.bills.update", [$bill->id]) }}" id="billForm" method="POST"
                  enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="bill_id" id="bill_id" value="{{ $bill->id }}">
                <div class="form-group {{ $errors->has('ticket_id') ? 'has-error' : '' }}">
                    <label for="ticket_id">{{ trans('cruds.bills.fields.ticket_name') }} <strong class="text-danger">*</strong></label>
                ...
                <input class="btn btn-success btn_create" type="button"
                ...
                <input class="btn btn-danger btn_submit btn_submit_and_sent_mail" style="display: none;"
                       type="button"
...
@section('scripts')
    <script>
            ...
            if (services && data['message'] == '') {
                for (let i = 0; i < services.length; i++) {
                    ...
                    html += '<div class="border d-flex align-items-center justify-content-start service_item_div" id="service_div_' + services[i].id + '">' +
                        ...
                        '<input type="number" name="cost_' + services[i].id + '" class="form-control cost_input" id="' + "cost_input_" + services[i].id + '" value="' + services[i].cost + '" onkeyup="if(this.value<0){this.value= this.value * -1}" min="0" disabled required>' +
                        '</div>' +
        ...
        function getServices(id = null) {
            if (id == null) {
                id = $(".ticket_id_change").val();
            }
            $.ajax({
                url: "{{ route('admin.tickets.getServices') }}",
                data: {id: id, isUpdate: 1},
                cache: false,
        ...
        $(document).ready(function () {
            ...
            $(".btn_create").on('click', function () {
                ...
                if (response == true) {
                    ...
                    data.push({name: "isUpdate", value: 1});
                    $.ajax({
