...
@section('content')
    ...
    <div class="col-12">
        <div class="bg-light rounded h-100 p-4">
            <h6 class="mb-4"> {{ trans('global.create') }} {{ trans('cruds.services.title_singular') }}</h6>
            <form action="{{ route("admin.services.store") }}" id="serviceForm" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <input type="checkbox" {{ (old('sub_category') == true) ? 'checked' : '' }} class="form-check-input" name="sub_category" id="sub_category">
                    <label for="sub_category" class="form-check-label">Add As Sub Category</label>
                </div>

                <div class="mb-3 parent_category" id="parent_category" @if(old('sub_category') != true)style="display: none;" @endif>
                    <div class="{{ $errors->has('parent_service_id') ? 'has-error' : '' }}">
                        <label for="parent_service_id">{{ trans('cruds.services.fields.parent_service_id') }} <strong class="text-danger">*</strong></label>
                        <select name="parent_service_id" id="parent_service_id" class="form-control select2">
                            ...
                            @foreach($parentServices as $id => $service)
                                <option value="{{ $id }}" {{ (old('parent_service_id') == $id) ? 'selected' : '' }}>{{ $service }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3 {{ $errors->has('name') ? 'has-error' : '' }}">
                    <label for="name">{{ trans('cruds.services.fields.name') }} <strong class="text-danger">*</strong></label>
                ...
                <div class="mb-3 {{ $errors->has('description') ? 'has-error' : '' }}">
                    ...
                    <textarea id="description" name="description"
                              class="form-control ">{{ old('description', isset($services) ? $services->description : '') }}</textarea>
                ...
                <div class="mb-3 {{ $errors->has('service_type') ? 'has-error' : '' }}">
                    ...
                    <div class="form-check form-check-inline px-2">
                        ...
                        <input type="radio" id="paid_service" name="service_type" class="form-check-input service_type" value="paid"
                               checked>
                    </div>

                    <div class="form-check form-check-inline px-2">
                        <label for="free_service" class="m-0 mx-2">Free</label>
                        <input type="radio" id="free_service" name="service_type" class="form-check-input service_type" value="free">
                    </div>
                ...
                <div class="mb-3 banner_div {{ $errors->has('attachments') ? 'has-error' : '' }}">
                ...
                <div class="mb-3 category_id_div {{ $errors->has('category_id') ? 'has-error' : '' }}">
                    <label for="category_id">{{ trans('cruds.services.fields.category_name') }} <strong class="text-danger">*</strong></label>
                    <select name="category_id" id="category_id" class="form-control select2" required>
                        <option value="" selected>Select Category</option>
                        @foreach($categories as $id => $category)
                            <option
                                value="{{ $category->id }}" {{ (old('category_id') == $category->id) ? 'selected' : '' }}>{{ $category->name .' - '. $category->id }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3 {{ $errors->has('currency') ? 'has-error' : '' }}">
                    <label for="currency">{{ trans('cruds.services.fields.currency') }} <strong class="text-danger">*</strong></label>
                ...
                <div class="mb-3 {{ $errors->has('is_payable') ? 'has-error' : '' }}">
                    ...
                    <input type="checkbox" id="is_payable" name="is_payable" value="1" class="form-check-input" checked>
                </div>
                <div class="mb-3 {{ $errors->has('cost') ? 'has-error' : '' }}">
                    <label for="cost">{{ trans('cruds.services.fields.cost') }} <strong class="text-danger">*</strong></label>
                    <input type="text" id="cost" name="cost" class="form-control"
                           value="{{ old('cost', isset($services) ? $services->cost : '') }}" >
                    @if($errors->has('cost'))
                        <em class="invalid-feedback">
                            {{ $errors->first('cost') }}
                        </em>
                ...
                <div id="member_cost_div" style="display: none;"
                    ...
                    <label for="member_cost">{{ trans('cruds.services.fields.member_cost') }} <strong class="text-danger">*</strong></label>
                    <input type="text" id="member_cost" name="member_cost" class="form-control"
                           value="{{ old('member_cost', isset($services) ? $services->member_cost : '') }}"
                           required>
                ...
                <div class="mb-3 contact_info_div {{ $errors->has('contact_info') ? 'has-error' : '' }}">
                    <label for="contact_info">{{ trans('cruds.services.fields.contact_info') }} <strong class="text-danger">*</strong></label>
                    <textarea id="contact_info" name="contact_info" required
                              class="form-control ">{{ old('description', isset($services) ? $services->contact_info : '') }}</textarea>
                ...
                <div class="mb-3 {{ $errors->has('status') ? 'has-error' : '' }}">
                    <label for="status">{{ trans('cruds.services.fields.status') }} <strong class="text-danger">*</strong></label>
                ...
                <div class="mb-3 assigned_user_div {{ $errors->has('assigned_user') ? 'has-error' : '' }}">
                    <label for="assigned_user">{{ trans('cruds.services.fields.assigned_to_users') }} <strong class="text-danger">*</strong></label>
                    <select name="assigned_user" id="assigned_user" class="form-control select2" required>
                        ...
                        @foreach($users as $id => $user)
                            <option
                                value="{{ $user->id }}" {{ (old('user_id') == $user->id) ? 'selected' : '' }}>{{ $user->name .' - '. $user->id }}</option>
                        @endforeach
                    </select>
                ...
                <button class="btn btn-primary serviceSave" type="button">{{ trans('global.submit') }}</button>
            </form>
        </div>
    </div>
@endsection
